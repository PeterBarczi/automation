...
HKA7="ssh-ed25519-cert-v01@openssh.com" #pb
HKA8="ssh-ed25519" #pb
...
# or users.
if [ ! "$ALLOW_GROUPS" ]; then ALLOW_GROUPS=LinuxUsers && SSH_GROUP=LinuxUsers; fi        # default

# Req 17:	The SSH Idle Timeout Interval must be configured to an adequate time.
if [ ! "$CLIENT_ALIVE_INT" ]; then CLIENT_ALIVE_INT=300; fi  # default
if [ ! "$CLIENT_ALIVE_CNT" ]; then CLIENT_ALIVE_CNT=0; fi    # default
...
if [ "${!REQ}" == "TRUE" ] || [ ! ${!REQ} ]; then
  ...
  # Test 2/2
  NUM=1
  if [ -n "$ALLOW_GROUPS" ]; then SSH="$SSH_GROUP $ALLOW_GROUPS"; fi
